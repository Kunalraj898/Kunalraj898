<!-- Hero Section -->
<h1 align="center">Hi 👋, I'm Kunal Verma</h1>
<h3 align="center">⚡ ECE Student | IoT Enthusiast | Aspiring Software Developer</h3>

<p align="center">
  Passionate about building real-world hardware projects and exploring software development.
</p>

---

<!-- Contact Links -->
<p align="center">
  <a href="mailto:kunalraj898@gmail.com">Email</a> •
  <a href="#">LinkedIn</a> •
  <a href="#">GitHub</a>
</p>

---

### 💡 About Me
- 🎓 B.Tech in **Electronics & Communication Engineering** (2023–2027)
- 🛠️ Interested in **IoT, Embedded Systems & Software Development**
- 🌱 Currently learning:
  - Java (OOP + Data Structures)
  - JavaScript (DOM basics)
  - Web Development fundamentals
- ⚡ Passionate about building **real-time hardware systems**

---

### 🧰 Tech Stack
<p align="center">
  <img src="https://img.shields.io/badge/Java-000?logo=openjdk&logoColor=white" />
  <img src="https://img.shields.io/badge/C-000?logo=c&logoColor=A8B9CC" />
  <img src="https://img.shields.io/badge/JavaScript-000?logo=javascript&logoColor=F7DF1E" />
  <img src="https://img.shields.io/badge/HTML-000?logo=html5&logoColor=E34F26" />
  <img src="https://img.shields.io/badge/CSS-000?logo=css3&logoColor=1572B6" />
  <img src="https://img.shields.io/badge/Arduino-000?logo=arduino&logoColor=00979D" />
  <img src="https://img.shields.io/badge/MATLAB-000?logo=mathworks&logoColor=FF6F00" />
  <img src="https://img.shields.io/badge/Git-000?logo=git&logoColor=F05032" />
</p>

---

### 🚀 Projects

#### 🔹 Automatic Car Parking System (Arduino)
- Built an automated parking system using **IR sensors & Arduino**
- Implemented vehicle detection logic
- Managed parking slot allocation system
- Learned real-time hardware interaction

#### 🔹 Water Quality Monitoring System (ESP32)
- Developed an **IoT-based monitoring system**
- Used sensors to measure water parameters
- Enabled real-time data collection
- Integrated multiple components for reliability

---

### 🛠️ Tools & Technologies
- Arduino IDE
- Proteus (Simulation)
- MATLAB (Signal Analysis)
- Visual Studio Code
- Git & GitHub (Basic)

---

### 🏆 Achievements
- 🏅 Participated in **Galgotias International Hackathon – TechFest 2025**
- 📜 Completed:
  - PCB Design (Coursera)
  - Cyber Security Fundamentals (Coursera)
- 💡 Built multiple IoT & hardware-based projects

---

### 👥 Communities
- 🤝 Member of **Google Developer Student Clubs (GDSC)**
- 💻 Member of **GeeksforGeeks Student Chapter (GFG)**

---

### 📊 GitHub Stats
<p align="center">
  <img height="160" src="https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&hide_title=true" />
  <img height="160" src="https://streak-stats.demolab.com?user=YOUR_USERNAME&hide_border=true" />
</p>

<p align="center">
  <img height="160" src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_USERNAME&layout=compact" />
</p>

---

### 🎯 Current Focus
- 📚 Strengthening **Data Structures & Algorithms**
- 🌐 Improving **Web Development skills**
- ⚡ Building more **IoT + Software integrated projects**

---

<p align="center">
  💡 “Learning by building real-world systems.”
</p>
